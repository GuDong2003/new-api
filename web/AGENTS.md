...
- **维护范围**：界面只提供英语 (`en`) 和简体中文 (`zhCN`，使用 `zh.json`)，英文是运行时回退语言。原有繁体中文偏好回退简体中文，其他旧语言偏好回退英文。仓库内旧的 `zh-TW`、`fr`、`ru`、`ja`、`vi` 文件保留归档，不再加载，也不再补充新译文。
- **写入规范**：新增键只能通过 `node scripts/add-missing-keys.mjs translations.json` 同时补齐 `en`、`zh`，保留现有译文与文件格式；`bun run i18n:sync` 只生成两种语言的对照报告，不得重写语言文件。
- **使用场景**
