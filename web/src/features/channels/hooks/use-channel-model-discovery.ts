...
type ChannelModelDiscoveryProps = {
  ...
  // Fork-only. Channels whose provider exposes no model-list endpoint (NovelAI)
  // ship a fixed catalogue instead of being unfetchable.
...
export function useChannelModelDiscovery(props: ChannelModelDiscoveryProps) {
  ...
  const fetch = useCallback(async () => {
    ...
    if (props.staticModels) {
      setState({ status: 'success', models: [...props.staticModels] })
      return
    }
  ...
  }, [props.enabled, props.request, props.staticModels])
