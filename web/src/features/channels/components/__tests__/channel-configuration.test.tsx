...
// The editor dialog is measured at x = 0..900, so the floating panel starts on
// its right.
...
test('on wide screens redirecting a fetched model opens the floating panel beside the editor and syncs the model list as the alias is typed', async () => {
  useWideScreen()
  const user = userEvent.setup()
  render(<ConfigurationHarness currentRow={editingChannel} />)
  await screen.findByDisplayValue('Existing channel')
...
test('on wide screens the quick options move into the header as a compact toggle strip and leave the form', async () => {
  ...
  expect(quick.closest('[data-slot="dialog-header"]')).not.toBeNull()
