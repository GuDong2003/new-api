...
it('offers database backups in the history type filter and queries them by type', async () => {
  const get = vi.spyOn(api, 'get').mockResolvedValue({
    data: { success: true, data: [], total: 0 },
  })
  const client = renderPanel()
  const typeFilter = await screen.findByRole('combobox', { name: 'Type' })
  expect(
    within(typeFilter).getByRole('option', { name: 'Database backup' })
  ).toHaveValue('database_backup')
  await userEvent.selectOptions(typeFilter, 'database_backup')
  await waitFor(() =>
    expect(get).toHaveBeenLastCalledWith(
      '/api/system-task/list',
      expect.objectContaining({
