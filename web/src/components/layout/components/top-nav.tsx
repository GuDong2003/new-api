...
export function TopNav({
  ...
  const expandedNav = (
    <nav
      className={cn(
        'items-center space-x-4 lg:space-x-4 xl:space-x-6',
        variant === 'expanded' ? 'flex' : 'hidden lg:flex',
        className
      )}
      {...props}
    >
  ...
  if (variant === 'expanded') return expandedNav

  return (
    <>
      <div className='lg:hidden'>{collapsedMenu}</div>
      {expandedNav}
    </>
  )
}
