...
func Register(c *gin.Context) {
	...
	if err := model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if inviteRequired {
			return model.ConsumeInviteCodeHashWithTx(tx, inviteCodeHash, cleanUser.Id, "password")
		}
		return nil
	}); err != nil {
		if errors.Is(err, model.ErrEmailAlreadyTaken) {
...
func DeleteSelf(c *gin.Context) {
	...
	// The managed avatar path has to be read while the row is still readable.
	deletedUser, _ := model.GetUserById(c.GetInt("id"), false)
	...
	if deletedUser != nil {
		if err := service.DeleteManagedAvatar(deletedUser.AvatarURL); err != nil {
			common.SysError(fmt.Sprintf("failed to remove avatar for deleted user %d: %v", deletedUser.Id, err))
		}
	}
	c.JSON(http.StatusOK, gin.H{
		"success": true,
