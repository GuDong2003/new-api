...
func TestChannelTestOpenAIChatCompatibility(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			// The default probe message is administrator-configurable, so pin it
			// here: this case asserts token-limit naming and stream options.
			request, ok := buildTestRequestWithMessage(tt.model, tt.endpoint, &model.Channel{}, tt.stream, "hi").(*dto.GeneralOpenAIRequest)
			require.True(t, ok)
