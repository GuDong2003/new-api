...
func TestManagedAvatarTargetAllowsSelfButRejectsPeer(t *testing.T) {
	...
	avatarRoute := fmt.Sprintf("/api/user/%d/avatar", target.Id)

	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	// A request is required: rejections translate their message, which reads
	// the Accept-Language header.
	c.Request = httptest.NewRequest(http.MethodPost, avatarRoute, nil)
	...
	peerContext.Request = httptest.NewRequest(http.MethodPost, avatarRoute, nil)
...
func TestManageUserQuotaConcurrentSnapshots(t *testing.T) {
	...
	for _, value := range []int{10, 20} {
		go func(value int) {
			adjustment, err := model.AdjustUserQuota(user.Id, user.Id+1, common.RoleRootUser, "add", value)
