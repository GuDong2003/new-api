...
// AdjustUserQuota commits a manual quota adjustment. An operator may act on a
// strictly lower role, on themselves, or on anyone when they are root — the
// same target rule the rest of the user-management endpoints apply.
func AdjustUserQuota(userID, operatorID, operatorRole int, mode string, value int) (*UserQuotaAdjustment, error) {
