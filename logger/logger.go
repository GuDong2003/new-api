...
// Every request path logs, so the rotation counter is written from many
// goroutines at once. The exact count still does not matter, but it has to be
// read and written atomically, and only one goroutine may start a rotation.
